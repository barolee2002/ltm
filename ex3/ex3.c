...
node *readData(node *root)
{
    ...
    if (file != NULL)
    {
        while (fscanf(file, "%s %s %d %s", temp.username, temp.password, &temp.status, temp.homepage) != EOF)
        {
            root = push(root, temp);
        }
    }
...
void saveData(node *root)
{
    ...
    while (root != NULL)
    {
        fprintf(file, "%s %s %d %s\n", root->user.username, root->user.password, root->user.status, root->user.homepage);
        root = root->pNext;
    }
    fclose(file);
}
...
void homepageWithDomain(node *root)
{
    char username[50];
    int isFound = 0;
    node *currentAccount = root;
    printf("__________Homepage with Domain__________\n");
    printf("Username: ");
    scanf("%s", username);
    while (currentAccount != NULL)
    {
    ...
    else
    {
        ...
        {
            printf("Homepage with Domain: %s\n", currentAccount->user.homepage);
        }
    }
}

void homepageWithIP(node *root)
{
    char username[50];
    int isFound = 0;
    node *currentAccount = root;
    printf("__________Homepage with IP__________\n");
    printf("Username: ");
    scanf("%s", username);
    while (currentAccount != NULL)
    {
...
int main()
{
    ...
    do
    {
        ...
        printf("6. Sign out\n");
        printf("7. Homepage with Domain\n");
        printf("8. Homepage with IP\n");
